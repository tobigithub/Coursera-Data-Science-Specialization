---
title: "The economic and health impact of weather events across the USA, 1996 - 2011"
...
```{r}
setwd(dir = "/home/khl4v/Github/Storms/")
...
In the subset of events after 1996 there are `r length(unique(stormdata$EVTYPE))` different types. Without domain knowledge it would be problematic to categorize all the rare events as on of the standard types, so this attempt was not made. However among the most common events are "THUNDERSTORM WIND" and "TSTM WIND" which seem to be the same which is why all event types including the term "TSTM WIND" will be categorized as "THUNDERSTORM WIND". The same applies for "WILDFIRE" as opposed to "WILD/FOREST FIRE" and "RIP CURRENT" as opposed to "RIP CURRENTS"
...
plotdata <- melt(plotdata, id.vars = c("Event Type"), value.name = "Amount", 
                 variable.name = "Damagetype")

ybreaks <- 150 * 2^(0:10)
healthp <- ggplot(plotdata, 
...
In general, all of the top 16 events cause more injuries than fatalities. One notable exception are flash floods which cause nearly as many fatalities as injuries so that the number of fatalities caused by flash floods is the third highest.
...
With respect to the economic damage tornadoes are less important. Floods 
caused roughly twice the damage of the second most dangerous event, hurricanes. The vast majority of the top events inflicts a much larger damage to property than to crops. The exception here are droughts which are by far the top cause of crop damage and which caused about 13$bn crop damage and just 1$bn property damage.
